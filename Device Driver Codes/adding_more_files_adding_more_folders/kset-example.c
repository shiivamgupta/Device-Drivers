...
static struct foo_attribute swp_attribute = __ATTR(dev_param4, 0664, b_show, b_store);
static struct foo_attribute aks_attribute = __ATTR(dev_param5, 0664, b_show, b_store);
...
static int example_init(void)
{
    ...
    baz_obj = create_foo_obj("device3");
	if (!baz_obj)
		goto baz_error;

    baz_obj = create_foo_obj("device4");
	if (!baz_obj)
		goto baz_error;
	return 0;
